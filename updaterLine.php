...
foreach ($data as $typeStr => $content) {
    ...
    try {
        ...
        $eventsFonbet  = $Parser->parseEvents($content, LINE_CONTENT_NAME);
    } catch ( \Exception $e ) {
        printf("[%s] Error: %s %s", date('d/M/Y H:i:s'), $e->getMessage(), PHP_EOL);
        exit(PHP_EOL);
    }
    ...
    printf("[%s] Info: Событий для обработки: [%s] %s", date('d/M/Y H:i:s'), count($eventsFonbet), PHP_EOL);
